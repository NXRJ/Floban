...
| Browser | `js/sync-docs.js` | Keeps this device's encoded Y.Doc per room in its own IndexedDB database, so a reload rejoins the room as the same document instead of a second lineage of the same board. |
