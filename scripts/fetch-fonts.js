...
(async () => {
  ...
  for (let i = 0; i < blocks.length; i++) {
    ...
    const url = src[1];
    const fileName = family[1].toLowerCase().replace(/[^a-z0-9]+/g, '-') + '-' + (i + 1) + '.woff2';
    faces.push({
      family: family[1],
      url,
      fileName,
  ...
  for (const face of faces) {
    ...
    fs.writeFileSync(path.join(outDir, face.fileName), data);
    out += '@font-face {\n';
    out += '  font-family: "' + face.family + '";\n';
    out += '  src: url("' + face.fileName + '") format("woff2");\n';
    out += '  font-weight: ' + face.weight + ';\n';
    out += '  font-style: ' + face.style + ';\n';
    if (face.range) out += '  unicode-range: ' + face.range + ';\n';
    out += '}\n';
    console.log('downloaded fonts/' + face.fileName + ' (' + data.length + ' bytes)');
  }
  fs.writeFileSync(path.join(outDir, 'fonts.css'), out);
  console.log('wrote fonts/fonts.css with ' + faces.length + ' faces');
